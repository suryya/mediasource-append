/**
 * @file In comparison with the segments mode, sequence is much easier, since the browser will handle the offsets 
 * automatically based on the decoded content. However, you are not really *supposed* to use this with separate files, 
 * and Chromium will likely deprecate support at some point (throws warning right now: `Warning: using MSE 'sequence' 
 * AppendMode for a SourceBuffer with multiple tracks may cause loss of track synchronization. In some cases, 
 * buffered range gaps and playback stalls can occur. It is recommended to instead use 'segments' mode for a multitrack SourceBuffer.`)
 */
